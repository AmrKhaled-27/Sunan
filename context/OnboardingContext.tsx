 ...
 * Positive scrolls the content up, negative scrolls it down. Targets taller
 * than the band are aligned to the top instead of being centred.
 */
function scrollDeltaFor(rect: TourRect, bounds: TourBounds): number {
...
export function OnboardingProvider({
	...
	const tabBarHeight =
		TAB_BAR_BASE_HEIGHT + Math.max(insets.bottom, TAB_BAR_MIN_INSET);
	...
	const measureStep = useCallback(
		async (index: number) => {
			...
			// The tab bar lives inside the navigator and cannot be registered, so
			// its rect is derived from the geometry declared in the tabs layout.
			...
			let rect = await resolveSettledRect(step.targetKey, isStale);
			if (isStale()) return;
